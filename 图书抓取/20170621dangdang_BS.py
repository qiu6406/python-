...
def getBookInfo(bookname,booklink):
	html = ''
	book = []
	rISBN = re.compile('国际标准书号ISBN：(\d+)</li>'.decode('utf-8'),re.DOTALL)
	rAuthor = re.compile('dd_name="作者">(.+?)</a>'.decode('utf-8'),re.DOTALL)
	rPress = re.compile('dd_name="出版社">(.+?)</a>'.decode('utf-8'),re.DOTALL)
	rPressDate = re.compile('出版时间:(.+?)&nbsp;</span>'.decode('utf-8'),re.DOTALL)
	...
	try:
		...
		if len(html) == 0:
			...
			if len(rISBN.findall(html)) == 0 or len(rAuthor.findall(html)) == 0 or len(rPress.findall(html)) == 0 or len(rPressDate.findall(html)) == 0 :
				...
				book.append( rPressDate.findall(html)[0].encode('gbk') )
